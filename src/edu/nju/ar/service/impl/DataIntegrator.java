...
public class DataIntegrator {
	...
	public void generateCinemaData(){
	...
	public void generateCinemaAndSessionData(){
		...
		for(session s1:ySession){
			...
			cinema c1=getter.getYupiaoerCinemaByCid(s1.getCid());
			movie m1=getter.getYupiaoerMovieByMid(s1.getMid());
			...
			finalSession.setMid(getter.getFinalMovieByName(m1.getName()).getId()); //movie信息已经集成，通过name获取finalMovie表中的id
			
			for(session s2:tSession){
				if(s2.getHall().equals(s1.getHall())&&s2.getDate().equals(s1.getDate())&&
			...
			for(session s3:bSession){
				if(s3.getHall().equals(s1.getHall())&&s3.getDate().equals(s1.getDate())&&
			...
			finalSession.setCid(getter.getFinalCinemaByName(m1.getName()).getId());
		...
		for(session s1:tSession){
			...
			double bPrice=0;
			session finalSession=new session();
			cinema c1=getter.getTaopiaopiaoCinemaByCid(s1.getCid());
			movie m1=getter.getTaopiaopiaoMovieByMid(s1.getMid());
			...
			integrator.addSession(finalSession, yPrice, tPrice, bPrice);
		      ...
		      //娱票儿中没有的百度有的电影院和场次
				for(session s1:bSession){
					...
					session finalSession=new session();
					cinema c1=getter.getBaiduCinemaByCid(s1.getCid());
					movie m1=getter.getBaiduMovieByMid(s1.getMid());
