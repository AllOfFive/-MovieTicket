...
<table style="margin-top:40px;text-align:left" class="table table-striped table-hover">
    ...
    <tbody>
     <c:forEach items="${movieList}" var="el">
    		<tr id="${el.id}">
    			<td>${el.name}</td>
    			<td>${el.type}</td>
    			<td>${el.last}</td>
    			...
    			<td>
    			<form action="searchCinemaList" method="post" id="searchCinemaListForm">
					<input type="text" style="display:none" id="movieId" name="movieId" value=${el.id}>  
    				<button type="submit" class="btn btn-info btn-xs">选择影院</button>
    			</form>
    			</td>
